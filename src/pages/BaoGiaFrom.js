...
const BaoGiaFrom = () => {
  ...
  const handlePrintQuote = async () => {
    ...
    printFrame.onload = function() {
      try {
        ...
        toast.error('Có lỗi xảy ra khi in báo giá');
        setIsPrinting(false);
      }
    };
  };
  ...
  const generatePrintContent = () => {
    const { quoteId, companyId, quoteDate, contractNumber, quoteDuration } = quoteInfo;
    const dateFormatted = quoteDate ? new Date(quoteDate).toLocaleDateString('vi-VN') : new Date().toLocaleDateString('vi-VN');
    ...
    return `
      ...
      <body>
        ...
        <div class="company-info">
          <h2>CÔNG TY DỊCH VỤ ABC</h2>
          <p>Địa chỉ: 123 Đường XYZ, Quận ABC, TP. HCM</p>
          <p>Điện thoại: 028 1234 5678 | Email: info@abc.com</p>
          <p>Mã số thuế: 0123456789</p>
        </div>
        
        <table class="quote-info">
          <tr>
            <td width="150"><strong>Số báo giá:</strong></td>
            <td>${quoteId || '[Chưa có ID]'}</td>
            <td width="150"><strong>Mã công ty:</strong></td>
            <td>${companyId || '[Chưa có ID công ty]'}</td>
          </tr>
          <tr>
            <td><strong>Ngày báo giá:</strong></td>
            <td>${dateFormatted}</td>
            <td><strong>Số hợp đồng:</strong></td>
            <td>${contractNumber || '[Chưa có số HĐ]'}</td>
          </tr>
          <tr>
            <td><strong>Hiệu lực:</strong></td>
            <td>${quoteDuration || '30'} ngày kể từ ngày báo giá</td>
            <td></td>
            <td></td>
          </tr>
        </table>
          ...
          <tbody>
            ${quoteItems.map((item, index) => `
              <tr>
                ...
                <td>${formatCurrency(item.afterDiscount)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        ...
        <table class="totals">
          <tr>
            ...
            <td align="right">${formatCurrency(totals.totalBeforeDiscount)} đ</td>
          </tr>
          <tr>
            <td>Tổng ưu đãi:</td>
            <td align="right">${formatCurrency(totals.totalDiscount)} đ</td>
          </tr>
          <tr>
            <td>Tổng tiền sau ưu đãi:</td>
            <td align="right">${formatCurrency(totals.totalAfterDiscount)} đ</td>
          </tr>
          <tr>
            <td>VAT (${paymentInfo.vatRate}%):</td>
            <td align="right">${formatCurrency(totals.vatAmount)} đ</td>
          </tr>
          <tr>
            <td>Chi phí duy trì:</td>
            <td align="right">${formatCurrency(parseFloat(paymentInfo.maintenanceFee) || 0)} đ</td>
          </tr>
          <tr>
            <td>Phí Public App:</td>
            <td align="right">${formatCurrency(parseFloat(paymentInfo.publicAppFee) || 0)} đ</td>
          </tr>
          <tr class="total-row">
            <td>TỔNG THANH TOÁN:</td>
            <td align="right">${formatCurrency(totals.grandTotal)} đ</td>
          </tr>
        </table>
        ...
        <div>
          ...
          <ol>
            <li>Báo giá có hiệu lực trong vòng ${quoteDuration || '30'} ngày kể từ ngày báo giá.</li>
            <li>Thanh toán 50% giá trị đơn hàng khi ký hợp đồng.</li>
            <li>Thanh toán 50% còn lại khi bàn giao và nghiệm thu.</li>
        ...
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button 
              ...
              disabled={isPrinting}
              className="bg-green-500 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-600 transition duration-300 flex items-center disabled:opacity-70"
            >
              {isPrinting ? (
                <>
                  <svg className="animate-spin h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
